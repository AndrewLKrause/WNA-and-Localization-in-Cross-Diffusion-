...
!parnames = {1:'a', 2:'b', 3:'c', 4:'d', 5:'h', 6:'delta'}
    ...
    SUBROUTINE FFFF(NDM,U,ICP,PAR,IJAC,F)
		...
		DOUBLE PRECISION sigma, lambda, d, p
		  ...
		  F(1) = U(3)
		  F(2) = U(4)
		  F(3) = U(1) - U(1)**2.0*U(2) - sigma*(U(1) - 1/U(2))**2.0 - p*((1 - U(1)**2.0)/(1 + U(1)**2.0)**2.0*U(3)*U(4) &
		  + U(1)/(1 + U(1)**2.0)*(U(1)**2*U(2) + sigma*(U(1) - 1/U(2))**2.0 - lambda)/d)
